...
describe("parseRequestBody", () => {
  test("returns object payloads unchanged", () => {
    const payload = {mode: "text", prompt: "hello"};
    expect(parseRequestBody(payload)).toEqual(payload);
  });

  test("unwraps single-element array payloads", () => {
    const payload = {mode: "text", prompt: "hello"};
    expect(parseRequestBody([payload])).toEqual(payload);
  });
  ...
  test("parses JSON string payloads", () => {
    const payload = {mode: "text", prompt: "hello"};
    expect(parseRequestBody(JSON.stringify(payload))).toEqual(payload);
  });

  test("rejects multi-element arrays", () => {
    expect(() => parseRequestBody([{mode: "text"}, {mode: "text"}])).toThrow();
  });
});
